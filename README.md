...
| [The New Rules of Context Engineering for Claude 5 Generation Models](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) | Anthropic removed over 80% of Claude Code's system prompt for newer models with no measurable loss on its coding evals. The resulting design shift is from blanket rules to model judgment, from tool-use examples to expressive interfaces, and from loading everything up front to selectively loaded skills and deferred tool definitions. The post suggests that a mature harness must delete obsolete scaffolding as the model changes, not only accumulate more instructions. |
...
| [Deep Agents v0.7](https://www.langchain.com/blog/deep-agents-v0-7) | LangChain | Removes the base system prompt and default todo middleware, cuts built-in tool descriptions by 43%, and lowers first-turn base input from about 6K to 2K tokens while a multi-model eval suite finds overall reward statistically unchanged. Todo planning becomes an opt-in tool for three identified task shapes; summarization thresholds, search truncation, and file pagination become explicit harness controls. A useful independent replication of the case for deleting stale scaffolding. |
...
| [SciCode-Verified: A Systematic Audit of Scientific Coding Benchmark Validity](https://arxiv.org/abs/2608.04975) | arXiv | Audits 65 scientific-programming problems and finds 263 defects across prompts, tests, and references; 192 correct solutions had been rejected. After repair, subproblem accuracy rises from 45–60% to 84–98% and main-problem accuracy from 9–27% to 69–92%. It is unusually direct evidence that benchmark plumbing can dominate the measured model result. |
