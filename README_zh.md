...
| [Claude 5 代模型的上下文工程新规则](https://claude.com/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models) | Anthropic 面向新模型删掉了 Claude Code 超过 80% 的 system prompt，而代码评测没有可测量的退步。设计重心随之从一刀切规则转向让模型判断、从工具调用示例转向表达力更强的接口、从预先塞满上下文转向按需加载技能和延迟加载工具定义。文章由此主张，成熟 harness 应随模型变化删除过时脚手架，而不只是不断叠加指令。 |
| [A Harness for Every Task：Claude Code 的动态工作流](https://claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code) | Anthropic 对程序化子智能体（subagent）编排的设计说明，归纳出六种可复用模式：classify-and-act、fan-out-and-synthesize、adversarial verification、generate-and-filter、tournament、loop-until-done。协调状态从单个模型上下文移到可重复运行多个隔离子智能体的脚本中。 |
...
| [Codex 0.147.0](https://github.com/openai/codex/releases/tag/rust-v0.147.0) | OpenAI | 把 Portable Agent Plugins 纳入聚合本地、个人、workspace 与远端来源的目录；新增 `--approve-for-me`、对 MCP 2026-07-28 的可选实现、远端压缩、对话分节，以及导入 Cursor 托管技能和 Claude/Cursor 对话。安全边界同步收紧：陌生本地项目必须显式信任，历史与回放中的 bearer token 会脱敏，plugin 策略更新失败时网络访问 fail closed。 |
| [Deep Agents v0.7](https://www.langchain.com/blog/deep-agents-v0-7) | LangChain | 删除基础 system prompt 和默认 todo middleware，把内置工具描述缩短 43%，首轮基础输入从约 6K 降到 2K token；多模型评测中整体 reward 在统计上没有显著变化。Todo 规划只在三类已识别任务形态中建议开启，摘要阈值、搜索截断和文件分页都变成显式 harness 控件。这是对“删除过时脚手架”很有价值的一次独立复现。 |
...
| [Warp Agent CLI](https://www.warp.dev/blog/introducing-the-warp-agent-cli-coding-agent) | Warp | 智能体建在 PTY multiplexer 上，而不是面向行的 shell wrapper，因此会话能跨目录切换和 SSH 跳转，并操作调试器、编辑器等交互式程序。原生子智能体可跨 Warp、Claude Code 与 Codex 委派，本地会话还能移交云端后在网页继续监控和引导。终端基础设施本身成为可移植 runtime 与控制面。 |
...
| [Poisoning Claude Code：一个 GitHub issue 如何击穿供应链](https://flatt.tech/research/posts/poisoning-claude-code-one-github-issue-to-break-the-supply-chain/) | 披露记录的是两条攻击路径，而不是一次已经发生的入侵：agent mode 把任意 GitHub App actor 当成可信；另一个官方低权限 triage 示例则可与高权限 tag-mode 工作流串联。两条路径中，被 prompt 注入的 Claude 都可能从 `/proc/self/environ` 读取 OIDC 请求凭据，再经 issue 更新外传。Anthropic 已在 `claude-code-action` v1.0.94 前完成缓解。 |
| [恶意 Skill 文件对代码智能体的风险评估](https://arxiv.org/abs/2608.05223) | 把智能体技能视为软件供应链，而不是无害 prompt 文本。作者把 471 条真实 shell 命令改写成 2,826 个外观正常、覆盖 11 类 MITRE ATT&CK tactic 的技能，并在 Gemini CLI 与 Qwen Code 上完成 5,629 次运行；报告的受利用率分别为 95.5–96.1% 与 71.6–74.0%，明确识别安全风险的运行仅占 1.99%。虽然预印本只使用一种合成攻击模板与两个 CLI，结果仍支持显式的技能准入、来源追踪、扫描与运行时最小权限。 |
...
| [LoopsBench：从 Harness Engineering 到循环工程的代码智能体评测](https://arxiv.org/abs/2608.00267) | arXiv | 评测持续开发而非一个 issue 对应一个终态。112 个任务被表示为依赖 DAG，包含 5,300 多个可独立测试的开发单元；flow-aware runtime 只在前置条件满足后释放测试，并把已完成节点保留为回归义务。最强组合（Opus 4.7 + Claude Code + 外层 continuation loop）完成 25% 的任务，而 agent 记录的计划只恢复了来源依赖结构的一部分。它是目录里第一个明确评测“agent loop 外面的 loop”的基准。 |
| [Active-SWE：没有 issue 报告时主动发现并修复缺陷](https://arxiv.org/abs/2608.04682) | arXiv | 拿掉多数仓库级基准默认赠送给智能体的高质量 issue 报告。它覆盖 1,663 个任务、六类缺陷与八种语言，要求智能体在没有报告引导时定位并修复已记录缺陷、处理多缺陷场景并发现额外有效缺陷。前沿系统在三种设置中都表现有限，说明主动发现与“读 issue 后打补丁”是不同能力。 |
| [SciCode-Verified：系统审计科学编程基准的有效性](https://arxiv.org/abs/2608.04975) | arXiv | 审计 65 道科学编程题，在 prompt、测试与参考答案中发现 263 处缺陷，其中 192 个正确解曾被拒绝。修复后，子问题准确率从 45–60% 升到 84–98%，主问题准确率从 9–27% 升到 69–92%。它直接说明基准管线本身可以主导最终测得的模型结果。 |
...
| [Rethinking Self-Evolving Agent Skills：反馈与搜索的作用](https://arxiv.org/abs/2608.02636) | arXiv | 说明技能演化更像稀疏、经验证筛选的搜索，而非单调学习：388 个候选中只有 55 个成为字节级不同的验证集最优；14 组设置中验证选出 11 个演化技能，其中九个提升测试表现，而且每个入选技能都使用了失败轨迹。并行采样几乎追平 SearchQA，却仍比演化后的 SpreadsheetBench 工作流落后 30.96 个百分点，收益明显依赖任务。 |
| [Learning Globally Reusable Skills for Coding Agents](https://arxiv.org/abs/2608.06153) | arXiv | 不再孤立改写单个技能，而是用 Skill Relation Graph 协同维护技能依赖，把局部更新聚合成可复用能力，并用 replay verification 拦截过拟合与回归。在 OpenHands、mini-SWE-agent 和内部工业智能体上评测缺陷揭示测试生成与误报过滤，把技能演化推进为有兼容性与回归检查的 skill bank 维护。 |
| [DCAS：解耦 CLI Agent 脚手架并跨脚手架内化规划能力](https://arxiv.org/abs/2608.06113) | arXiv | 在 OpenHands 轨迹上微调的模型从 OpenHands 下的 52.6% 降到 OpenCode 下的 8.4%，而基座模型分歧更小。DCAS 拦截 API 流量以配对 CLI 脚手架与其他后端，并收集 576 条具备规划信息的 Claude Code 轨迹；规划与执行联合微调在自主规划时达到 55.8%，并迁移到未见过的 OpenCode（49.4%）和 mini-SWE-agent（42.4%）。这项单基准研究直接警告轨迹训练会过拟合脚手架协议。 |
| [Ledger：代码智能体的有状态 Harness Middleware](https://arxiv.org/abs/2608.00808) | arXiv | 用外部任务账本记录证据、依赖与已验证进度，不增加模型调用。在完整 500 道 SWE-bench Verified 上，GPT-5 mini 从 56.2% 升到 64.2%、成本下降 28.9%；MiniMax M2.5 从 75.8% 升到 81.0%、成本下降 31.8%；Codex 也提升 3.4 个百分点并降本 24.4%。这是显式状态作为 harness middleware 而非 prompt 散文的强证据。 |
| [The Resume Contract：失败后的 Durable Agent Workflow 测试](https://arxiv.org/abs/2608.03836) | arXiv | 为 checkpoint/resume 行为定义六条形式化性质，对 740 万个状态做模型检查，并实测五个 workflow framework。结果显示 crash recovery 可能重跑已经持久化的工作、重复副作用，甚至把一次逻辑 resume 放大成每个并发进程各执行一次。“持久”因此需要明确的 resume contract，而不只是 checkpoint API。 |
